...
context('WhenAllAreDoneToggleAllTodos', () => {
    beforeEach(() => {
    ...
    it('allTodosWereSetToOpen ', () => {
    	...
    	nonDeterministicValues.push(nonDeterministicValue);
    	AppUtils.httpPut(`/api/test/non-deterministic/system-time?uuid=${testId}_toggle-all-2&system-time=${new Date('2020-10-10T17:58:37.000Z').toISOString()}`)
    	localStorage.setItem('nonDeterministicValues', JSON.stringify(nonDeterministicValues));
    	
    	ScenarioUtils.getCypressFor(TodoActionIds.toggleAll, ).should(() => {
    		ScenarioUtils.wait(0, 4).should(() => {
	            const appState = JSON.parse(localStorage.getItem('appState'))
	            expect(appState.todos.todoList, "allTodosWereSetToOpen").to.eql([
	            	{ 
