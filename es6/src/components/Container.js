...
import {
	div, error, generic, h3, h5, span, a, br, hr, p, ul, li, input, todos, footer, h1, spinner, newTodoInput
} from "../../gen/components/ReactHelper";
import Utils from "../../gen/ace/Utils";
import {toggleAll} from "../../gen/todo/ActionFunctions";

export function uiElement(attributes) {
	const itemCount = attributes.todos && attributes.todos.todoList ? attributes.todos.todoList.filter((todo) => todo.done !== true).length : 0;
	return div({}, [
		...
		div({class: "learn-bar body"},
			[
				...
				div({},
					[
						generic("section", {class: "todoapp"},
							[
								generic("header", {class: "header"},
									[
										h1({},
											[
												span({}, ["todos"]),
										...
										newTodoInput({...attributes.newTodoInput})
									]
								),
				...
				generic("footer", {class: "info"},
					[
						p({}, ["Double-click to edit a todo"]),
						p({}, ["Created by Annette Pohl"]),
					]
				)
			])
	]);
