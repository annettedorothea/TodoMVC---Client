...
module.exports = {
    ...
    invokeAction: async function (driver, action, args) {
        ...
        if (TodoActionIds.newTodoKeyPressed === action) {
            ...
            if (args[0] === 13) {
                await pressEnter(driver, "newTodoInput")
            }
            if (args[0] === 27) {
                await pressEsc(driver, "newTodoInput")
            }
            await this.waitInMillis(500);
        }
...
async function pressEnter(driver, id) {
    await driver.findElement(By.id(id)).sendKeys(Key.ENTER);
}

async function pressEsc(driver, id) {
    await driver.findElement(By.id(id)).sendKeys(Key.ESCAPE);
}
